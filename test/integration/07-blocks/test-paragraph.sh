#!/bin/bash
# Test: Paragraph block

generate_jwt "posts:write"

body='{
    "title": "Paragraph Block Test",
    "status": "draft",
    "content": {
        "blocks": [
            {"type": "paragraph", "content": "This is a simple paragraph."},
            {"type": "paragraph", "content": "This is another paragraph."}
        ]
    }
}'
response=$(api_post "/posts" "$body")
status=$(get_status)

assert_status "201" "$status" "Create post with paragraph blocks returns 201"
assert_json_exists "$response" ".content" "Response contains content"
assert_contains "This is a simple paragraph" "$response" "Content contains first paragraph"

post_id=$(echo "$response" | jq -r '.id // .post_id // empty')
if [[ -n "$post_id" ]]; then
    CREATED_POSTS+=("$post_id")
fi
